...
BaseDataLayer<Dtype>::BaseDataLayer(const LayerParameter& param) :
		Layer<Dtype>(param), transform_param_(param.transform_param()), data_transformer_(
				transform_param_) {
}
...
void BaseDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	if (top->size() < 3)
		output_labels_alt_ = false;
	else
		output_labels_alt_ = true;

	if (top->size() < 4)
		output_labels_alt1_ = false;
	else
		output_labels_alt1_ = true;
	...
	// add by zhangll, if we have mean_pixel, then set mean_pixel
	CHECK(
			!(transform_param_.has_mean_file()
					&& transform_param_.has_mean_pixel()))
			<< "cannot set meanfile and meanpixel at the same time";

	if (transform_param_.has_mean_pixel()) {
		const Dtype mean_pixel = transform_param_.mean_pixel();
		LOG(INFO) << "Set mean pixel:" << mean_pixel;
		BlobProto blob_proto;
		data_mean_.Reshape(1, datum_channels_, datum_height_, datum_width_);
		for (int i = 0; i < data_mean_.count(); i++) {
			data_mean_.mutable_cpu_data()[i] = mean_pixel;
		}
	}
...
void BasePrefetchingDataLayer<Dtype>::LayerSetUp(
	...
	if (this->output_labels_alt_) {
		this->prefetch_label_alt_.mutable_cpu_data();
	}

	if (this->output_labels_alt1_) {
		this->prefetch_label_alt1_.mutable_cpu_data();
	}
...
void BasePrefetchingDataLayer<Dtype>::Forward_cpu(
	...
	if (this->output_labels_alt1_) {
		caffe_copy(prefetch_label_alt1_.count(),
				prefetch_label_alt1_.cpu_data(), (*top)[3]->mutable_cpu_data());
	}
