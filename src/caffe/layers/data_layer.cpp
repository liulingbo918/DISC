...
void DataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	case DataParameter_DB_LEVELDB: {
		...
		leveldb::Status status = leveldb::DB::Open(options,
				this->layer_param_.data_param().source(), &db_temp);
		CHECK(status.ok()) << "Failed to open leveldb "
									<< this->layer_param_.data_param().source()
...
void DataLayer<Dtype>::InternalThreadEntry() {
	...
	for (int item_id = 0; item_id < batch_size; ++item_id) {
		...
		case DataParameter_DB_LMDB:
			CHECK_EQ(
					mdb_cursor_get(mdb_cursor_, &mdb_key_, &mdb_value_,
		...
		if (this->output_labels_) {
			...
			for (int i = 0; i < datum.label().size(); i++) {
				top_label[item_id*datum.label().size()+i] = datum.label(i);
			}
		}
