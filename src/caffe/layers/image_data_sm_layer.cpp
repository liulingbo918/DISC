...
ImageDataSMLayer<Dtype>::~ImageDataSMLayer<Dtype>() {
	this->JoinPrefetchThread();
}

template<typename Dtype>
void ImageDataSMLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	Datum datum;
	CHECK(
			ReadImageToDatumSM(lines_[lines_id_].first,
	...
	if (crop_size > 0) {
		...
		/*(*top)[0]->Reshape(batch_size, 1, 1,
						1);
				this->prefetch_data_.Reshape(batch_size, 1,
						1, 1);*/
	}
	...
	(*top)[1]->Reshape(batch_size, label_size * label_size, 1, 1);
	this->prefetch_label_.Reshape(batch_size, label_size * label_size, 1, 1);
	// datum size
...
template<typename Dtype>
void ImageDataSMLayer<Dtype>::ShuffleImages() {
...
template<typename Dtype>
void ImageDataSMLayer<Dtype>::InternalThreadEntry() {
	Datum datum;
	...
	for (int item_id = 0; item_id < batch_size; ++item_id) {
		...
		if (!ReadImageToDatumSM(lines_[lines_id_].first,
				lines_[lines_id_].second, new_height, new_width, &datum)) {
			continue;
		}
...
INSTANTIATE_CLASS(ImageDataSMLayer);
