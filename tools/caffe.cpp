...
int test() {
	...
	const std::string image_input_list = "DISC_Data/DISC_test.list";
	const std::string result_save_path = "DISC_Data/DISC_Output/";
	vector < std::string > filenames;
	std::string filename;
	std::ifstream file(image_input_list.c_str());
	while (file >> filename) {
	...
	cv::Mat smap(img_size, img_size, CV_8UC1);
	cv::Mat remap;
	float loss = 0;
	int batchSize = 1;
	for (int i = 0; i < FLAGS_iterations; ++i) {
		...
		Blob<float>* pre_result = (*(caffe_net.bottom_vecs().rbegin()))[0];
     	
		for (int j = 0; j < batchSize; ++j) {

			for (int h = 0; h < img_size; h++) {
				for (int w = 0; w < img_size; w++) {
					if (pre_result->data_at(j, 0, h, w) <= -1) {
						smap.at<unsigned char>(h, w) =
								(static_cast<uint8_t>(0));
					} else if (pre_result->data_at(j, 0, h, w) >= 1) {
						smap.at<unsigned char>(h, w) =
								(static_cast<uint8_t>(255));
					} else {
						smap.at<unsigned char>(h, w) = (static_cast<uint8_t>((1
								+ pre_result->data_at(j, 0, h, w)) * 255 / 2));
					}
				}
			}

			LOG(INFO) << result_save_path + filenames[i * batchSize + j];
			cv::imwrite( result_save_path + filenames[i * batchSize + j] + "_DISC.png", smap );
		}
	}
	return 0;
}
...
int time() {
	...
	for (int i = layers.size() - 1; i >= 0; --i) {
		...
		LOG(INFO) << layername << "\tbackward: " << timer.MilliSeconds()
				<< " milliseconds.";
	}
